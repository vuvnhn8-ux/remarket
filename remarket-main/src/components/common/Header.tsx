        ...
        <div className={`flex items-center justify-between transition-all duration-300 ${collapsed ? 'gap-2' : 'gap-6'}`}>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            >
              <img
                src="/favicon.svg"
                alt="ReMarket logo"
                className={`shrink-0 transition-all duration-300 ${collapsed ? 'w-6 h-6' : 'w-10 h-10'}`}
                width={40}
                height={40}
              />
              <div className="flex flex-col">
                <span className={`font-black tracking-tighter text-emerald-600 leading-none transition-all duration-300 ${collapsed ? 'text-base' : 'text-2xl'}`}>
                  {t('brand.title')}
                </span>
                {!collapsed && (
          ...
          <div className={`flex-1 hidden md:flex justify-center transition-all duration-300 ${collapsed ? 'max-w-md' : 'max-w-xl'}`}>
            <div className="relative flex items-center w-full max-w-xl">
              ...
              <input
                ...
                className={`w-full bg-gray-100 border-none rounded-full pl-10 pr-28 text-sm text-gray-800 placeholder-gray-400 focus:bg-white focus:ring-2 focus:ring-gray-300 transition-all outline-hidden ${collapsed ? 'py-1 text-xs' : 'py-2'}`}
              />
          ...
          <div className={`flex items-center transition-all duration-300 ${collapsed ? 'gap-1.5 sm:gap-2' : 'gap-2 sm:gap-4'}`}>
            ...
            <button
              ...
              className={`flex items-center bg-gray-900 text-white rounded-full cursor-pointer hover:bg-gray-800 transition-colors text-xs font-semibold gap-1.5 ${collapsed ? 'p-1.5' : 'px-4 py-1.5'}`}
              title={t('nav.myPage')}
            >
